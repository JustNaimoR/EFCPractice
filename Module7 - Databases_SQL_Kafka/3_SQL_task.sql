select * from artist_credit where name='The Beatles';


select * from release r join artist_credit ac on r.artist_credit=ac.id where r.name='The Beatles';



select * from area where name='United Kingdom';
select * from artist where end_date_year='1970';

create view releases_full_info_view as
    -- select rel.name from release rel
--          join release_status rel_stat on rel.status = rel_stat.id
--          join medium med on med.release = rel.id
--          join medium_format med_form on med.format = med_form.id
--          join release_info rel_info on rel.id = rel_info.release
-- group by rel.name;
-- drop view releases_full_info_view;
--
-- select * from releases_full_info_view rel;
...
select * from artist where name ='The Beatles';
select * from artist_credit where name ='The Beatles';
select * from artist_credit_name a join artist_credit b on a.artist_credit=b.id where a.name ='The Beatles';
